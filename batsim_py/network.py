...
from batsim_py.resource import *
    ...
    if "type" in msg:
        if msg["type"] == EventType.JOB_SUBMITTED:
            return JobSubmittedEvent(
                msg["data"]["job"].get("subtime", msg["timestamp"]),
                msg["data"]["job"]["id"],
                msg["data"]["job"]["profile"],
                msg["data"]["job"]["res"],
                msg["data"]["job"]["walltime"],
                msg["data"]["job"].get("user", ""),
            )
...
class SetResourceStateRequest(Request):
    class __data__(Serializable):
        ...
        def update(self, new_resources):
            assert isinstance(new_resources, list)
            res = ProcSet.from_str(self.resources)
            self.resources = str(res.union(*new_resources))

    def update_resources(self, new_resources):
        self.data.update(new_resources)
...
class SimulatorEventHandler(ABC):
