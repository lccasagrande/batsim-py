...
def plot_simulation_graphics(output_fn, show=True):
    jobs = JobSet.from_csv(output_fn + '_jobs.csv')
    mstates = MachineStatesChanges(output_fn+'_machine_states.csv').df
    pstates = PowerStatesChanges(output_fn+'_pstate_changes.csv')
    energy = pd.read_csv(output_fn+'_consumed_energy.csv')
    df = energy.drop_duplicates(subset='time')
    df = df.drop(['event_type', 'wattmin', 'epower'], axis=1)
    diff = df.diff(1)
    diff.rename(columns={'time': 'time_diff',
                         'energy': 'energy_diff'}, inplace=True)
    ...
    plot_load(jobs.utilisation, jobs.MaxProcs,
              load_label='Resource Load', ax=ax_list[0])
    plot_load(jobs.queue, jobs.MaxProcs,
              load_label='Queue Load', ax=ax_list[1])
