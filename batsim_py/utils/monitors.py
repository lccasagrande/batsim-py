...
class SimulatorMonitor(SimulatorEventHandler):
    @abstractmethod
    def to_csv(self, fn):
        raise NotImplementedError


class JobMonitor(SimulatorMonitor):
...
class JobsStatsMonitor(SimulatorMonitor):
    ...
    def _start_job(self, timestamp, alloc, job):
        ...
        job.start(timestamp)
        if self.qos_stretch and job.waiting_time / job.walltime >= self.qos_stretch:
...
class ResourceStatsMonitor(SimulatorMonitor):
    def __init__(self, simulator):
        super().__init__(simulator)
...
class NodeStatsMonitor(SimulatorMonitor):
    def __init__(self, simulator):
        super().__init__(simulator)
...
class SchedulerStatsMonitor(SimulatorMonitor):
    def __init__(self, simulator, qos_stretch=None):
        super().__init__(simulator)
...
class ResourceStatesEventMonitor(SimulatorMonitor):
    def __init__(self, simulator):
        super().__init__(simulator)
        self.last_resource_state = self._platform = self._jobs_running = None
        self.info = {
...
class ResourcePowerStatesEventMonitor(SimulatorMonitor):
    def __init__(self, simulator):
        super().__init__(simulator)
        self._platform = None
...
class PowerEventMonitor(SimulatorMonitor):
    def __init__(self, simulator):
        super().__init__(simulator)
        self.last_resource_state = self._platform = self._jobs_running = None
        self.info = {
    ...
    def _update_resource_state(self, timestamp, resources, event_type, new_state=None, new_pstate_id=None):
        ...
        if self.info['time'] and self.info['time'][-1] == timestamp and self.info['event_type'][-1] == event_type:
            self.info['energy'][-1] = energy
            self.info['epower'][-1] = epower
        else:
            self.info['time'].append(timestamp)
